...
class DotProductRandomProjectionModelTest {
  @Test
  def testGetBandedHashes(): Unit = {
    ...
    val model = new DotProductRandomProjectionModel(hashFunctions = rps)
    Assert.assertEquals(model.distance.metric, DistanceMetric.dot)
